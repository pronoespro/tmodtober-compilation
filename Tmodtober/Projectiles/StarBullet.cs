    ...
    public class StarBullet:ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			for (int k = 0; k < Projectile.oldPos.Length; k++)
			{
				...
				Main.EntitySpriteDraw(texture, drawPos, null, color, Projectile.rotation + (float)Math.PI / 2, drawOrigin, Projectile.scale, SpriteEffects.None, 0);
			}
		...
		public override void OnKill(int timeLeft)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				Vector2 _offset = new Vector2(MathF.Cos(i / 2f * MathHelper.PiOver2), -MathF.Sin(i / 2f * MathHelper.PiOver2));

				Projectile.NewProjectile(_s, Projectile.Center+_offset*300,-_offset*20,ProjectileID.Starfury,Projectile.damage*2,2,Projectile.owner);
			}
